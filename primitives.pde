...
void dessinePlanete(float res, float rayon, boolean fond) {
  ...
  for (int i = 0; i < 2 * nbFaces; i++) {
    ...
    vertex(0, -( rayon + altitude) * echelle, 0);
    for (int j = 1; j < nbFaces; j++) {
      ...
      vertex(x[i][j] * r, y[i][j] * r, z[i][j] * r);
      ...
      if (i < 2 * nbFaces - 1) vertex(x[i+1][j] * r, y[i+1][j] * r, z[i+1][j] * r);
      else vertex(x[0][j] * r, y[0][j] * r, z[0][j] * r);
    }
